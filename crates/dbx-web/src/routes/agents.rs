    ...
    while let Ok(Some(field)) = multipart.next_field().await {
        ...
        if name == "dbType" {
            ...
            if !jar_name.to_lowercase().ends_with(".jar") {
                return Err(AppError::from("Only .jar files can be imported".to_string()));
            }
            jar_data = Some(field.bytes().await.map_err(|e| AppError::from(e.to_string()))?.to_vec());
        }
    }

    let db_type = db_type.ok_or_else(|| AppError::from("Missing dbType field".to_string()))?;
    let data = jar_data.ok_or_else(|| AppError::from("No file uploaded".to_string()))?;
