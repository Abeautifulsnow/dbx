...
pub async fn ai_agent_stream(
    ...
    let agent_ctx = AgentLoopContext { state: state.inner().clone(), connection_id, database, db_type: parsed_db_type, schema_cache: None /* None = use AppState-managed TTL cache automatically; Some(arc) = inject a specific cache instance (e.g., for testing) */ };
